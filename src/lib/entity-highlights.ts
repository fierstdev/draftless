...
export type EntityHighlightPresetId =
  | "hidden"
  | "subtle"
  | "balanced"
  | "strong";
...
export const DEFAULT_ENTITY_HIGHLIGHT_VISIBILITY: EntityHighlightVisibility =
  "visible";
export const DEFAULT_ENTITY_HIGHLIGHT_OPACITY = 80;
...
export function getInitialEntityHighlightVisibility(): EntityHighlightVisibility {
  const storedVisibility = localStorage.getItem(
    ENTITY_HIGHLIGHT_VISIBILITY_KEY,
  );
  return storedVisibility === "hidden"
    ? "hidden"
    : DEFAULT_ENTITY_HIGHLIGHT_VISIBILITY;
}

export function getInitialEntityHighlightOpacity(): number {
  const storedOpacity = Number(
    localStorage.getItem(ENTITY_HIGHLIGHT_OPACITY_KEY),
  );
...
export function getEntityHighlightPresetId(preferences: {
  ...
  const preset = ENTITY_HIGHLIGHT_PRESETS.find(
    (candidate) =>
      candidate.visibility === "visible" && candidate.opacity === opacity,
  );
