...
export function AppHeader({ projectDoc }: AppHeaderProps) {
  ...
  const activeFileTitle = activeFileId
    ? (new ProjectManager(projectDoc)
        .getAll()
        .find((file) => file.id === activeFileId)?.title ?? currentDoc.title)
    : currentDoc.title;
              ...
              <BreadcrumbList>
                <BreadcrumbItem>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-auto shrink-0 px-2 font-normal text-muted-foreground hover:text-foreground"
                    onClick={() => setCurrentDoc(null)}
                  >
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <span className="block max-w-60 truncate font-semibold text-foreground lg:max-w-[320px] xl:max-w-[420px] 2xl:max-w-[520px]">
                    {currentDoc.title}
                  </span>
                </BreadcrumbItem>
              </BreadcrumbList>
