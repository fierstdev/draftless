...
export function Library() {
  ...
  const resetDemoProject = async () => {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage(
        "Couldn't refresh the demo story just yet. Please try again in a moment.",
      );
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage(
        "Couldn't delete this story yet. Close any open Draftless tabs for it and try again.",
      );
      setDocToDelete(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-5 sm:p-8 animate-in fade-in duration-500 flex flex-col">
      <div className="max-w-6xl mx-auto space-y-6 sm:space-y-10 lg:space-y-12 w-full flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-muted-foreground text-sm leading-relaxed">
                A writing studio for brave drafts
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center sm:gap-3">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="w-full hidden sm:flex sm:w-auto gap-2 bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg justify-center">
              ...
              <div className="space-y-5">
                <h2 className="mb-3 text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl lg:text-5xl">
                  ...
                  <span className="text-primary">
                    Find your way back anytime.
                  </span>
                </h2>
                <p className="max-w-md text-base sm:text-lg text-muted-foreground leading-relaxed">
                  Draftless gives you room to explore. Try alternate scenes,
                  compare saved versions, keep your story notes close at hand,
                  and ask AI for help when two drafts pull in different
                  directions.
                </p>
              ...
              >
                <CardHeader className="pb-3 p-4 sm:p-5">
                  <div className="flex justify-between items-start">
                    <div className="mb-3 rounded-xl bg-primary/10 p-2 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground sm:p-2.5">
                ...
                <CardFooter className="border-t border-border/50 bg-muted/20 px-4 sm:px-5 py-3 text-[10px] text-muted-foreground flex items-center gap-3">
        ...
        <Dialog
          open={!!docToRename}
          onOpenChange={(open) => !open && setDocToRename(null)}
        >
          <DialogContent className="bg-card border-border sm:max-w-[425px]">
            <DialogHeader>
...
function FeatureCard({ icon: Icon, title, desc, color }) {
  return (
    <div className="flex gap-3 sm:gap-4 p-4 rounded-xl border border-border bg-card hover:bg-accent/50 transition-colors">
      <div
        className={`p-2.5 sm:p-3 rounded-lg bg-background h-fit border border-border/50 shadow-sm ${color}`}
      >
